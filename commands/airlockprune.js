...
async execute(message, args, client) {
    if (config.roleAirlock && config.airlockChannel) {
      ...
      airlockChannels = await client.channels.resolve().filter(channel => channel.name.includes(AIRLOCK_CHANNEL) && channel.viewable);
      ...
      message.guild.roles.cache.get(AIRLOCK_ROLE_ID).members.forEach(u => { 
        ...
        let entry = `> - <@${u.id}> (${u.user.username}#${u.user.discriminator}) joined `; //${moment(u.joinedAt).format('MMM Do')} displays Feb 23 if we need it
        if (daysSinceJoin > 1) {
