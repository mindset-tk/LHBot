...
async function restoreMessages(config, client, callback) {
  // we changed some data structures. Run a quick clean up.
  for (const [, value] of Object.entries(global.dataLog)) {
    for (const [, v] of Object.entries(value)) {
      if (v.lastMessageID) {
        v.lastMessageId = v.lastMessageID;
        delete v.lastMessageID;
      }
    }
  }
  ...
  do {
    ...
    for (let g of await client.guilds.cache) {
      ...
      for (let gc of g.channels.cache) {
        ...
        if ((gc.type === 'GUILD_TEXT' || gc.type === 'GUILD_NEWS') && !global.dataLog[g.id][gc.id] && gc.permissionsFor(g.me).has(['VIEW_CHANNEL', 'READ_MESSAGE_HISTORY']) && !(config.airlockChannel != '' && gc.name.includes(config.airlockChannel))) {
